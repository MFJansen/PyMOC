...
if __name__ == '__main__':
    ...
    bs=0.02+deltabs; bs_north=-0.001+deltabs; bminSO=0.0+deltabs;
    ...
    l=2.e6;
    y=np.asarray(np.linspace(0,l, 51)); dy=y[1]
    
    # target ("equilibrium") surface buoyancy profile for restoring
    bs_SO_eq=0.*y+bminSO;
    alpha=(1.-np.cos(np.pi*(l-y[5])/7.4e6))
    bs_SO_eq[6:]= (bs-bminSO)*(1.-np.cos(np.pi*(y[6:]-y[5])/7.4e6))/alpha +bminSO 
    
    # fixed flux and restoring mask for SO ML:
    surflux=0.*y;surflux[1:6]=-Bloss;
    rest_mask=0.*y;rest_mask[6:-1]=1.;
    ...
    kapgcm=np.array([1.2e-4, 0.882e-4,0.544e-4,0.393e-4,0.305e-4,0.235e-4,0.207e-4,
    ...
    zgcm=-1e3*np.array([0.0, 0.0200, 0.045, 0.075, 0.110, 0.150, 0.200,
               0.260, 0.330, 0.410, 0.500, 0.600, 0.720,
               0.860, 1.020, 1.200, 1.400, 1.600, 1.800,
    ...
    def kappa(z):
        return np.interp(-z,-zgcm,kapgcm) #notice that coordinate has to be monotonically increasing, hence we use -z
    def kappaeff(z): # effective diffusivity profile with tapering in BBL
        return np.interp(-z,-zgcm,kapgcm)*(1.-np.maximum(-4000.-z+500.,0.)/500.)**2  #notice that coordinate has to be monotonically increasing, hence we use -z
    ...
    bs_SO[-1]=bs;# make sure buoyancy at northern end of channel (serves as BC for diffusion) equals surface b in basin
    if fixbSO: 
       bs_SO[:-1]=bs_SO[:-1]+deltabs  # instanteneously adjust surface b in SO
    
    # create vert. adv-diff column model instance for basin
    basin= Column(z=z,kappa=kappaeff,Area=A_basin,b=b_basin,bs=bs,bbot=b_basin[0])
    # create adv-diff column model instance for basin
    north= Column(z=z,kappa=kappaeff,Area=A_north,b=b_north,bs=bs_north,bbot=b_north[0])
    
    # create hor. adv-diff model instance for SO/channel ML
    channel=SO_ML(y=y,h=h,L=L,Ks=kaps, surflux=surflux,rest_mask=rest_mask,
                      b_rest=bs_SO_eq,v_pist=vpist,bs=bs_SO)          
    ...
    for ii in range(0, total_iters):  
       ...
       if ii%MOC_up_iters==0:
           ...
           AMOC.update(b1=basin.b,b2=north.b) # Notice that, even with fixed PsiN,
       ...
       if Psi_res_b[1]>0 and north.b[0]<basin.b[1] and north.b[0]<channel.bs[0]:
          # bottom water coming in from the north  
          basin.bbot=north.b[0]
          basin.kappa=kappaeff         
       elif PsiSO.Psi[1]>=0: # notice that this implies neither of the two above
    ...
    if diagfile is not None:
       np.savez(diagfile, AMOC_save, AMOC_b_save,  b_basin_save,
                b_north_save, bs_SO_save, z, bgrid_save, y, Psi_SO_save)      
